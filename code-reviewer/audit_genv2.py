...
def overwrite_original_file_with_code(
    md_file_path: str, original_file_path: str
) -> None:
    ...
    try:
        ...
        if code_content:
            ...
            logging.info(
                f"Commented out the original content and appended new code to {original_file_path}"
            )
        else:
            logging.error(
...
def main() -> None:
    ...
    choice = (
        input("Do you want to analyze the Python or Ruby directories? (python/ruby): ")
        .strip()
        .lower()
    )
    ...
    subdirectory_choice = input(
        "Select a subdirectory or 'All' to process (enter the number): "
    ).strip()
    ...
    if selected_subdirectory == "All":
        directories_to_process = [
            os.path.join(base_directory, subdir)
    ...
    for temp_dir in existing_temp_dirs:
        ...
        if os.path.exists(files_to_process_path):
            ...
            if files:
                input(
                    f"Press enter to continue processing from {temp_dir}. {len(files)} files left."
                )
