...
class TestConfigManager(unittest.TestCase):
    @patch(
        "uxr_analysis.config_manager.os.path.join",
        return_value="mocked_path/uxr_config.json",
    )
    ...
    @patch(
        "builtins.open", new_callable=mock_open, read_data=json.dumps({"key": "value"})
    )
    @patch(
        "uxr_analysis.config_manager.ConfigManager.get_config_file",
        return_value="mocked_path/uxr_config.json",
    )
    ...
    def test_load_config_file_not_found(
        self, mock_logger_error, mock_get_config_file, mock_open_file
    ):
        output_dir = "mocked_path"
        with self.assertRaises(FileNotFoundError):
            ConfigManager.load_config(output_dir)
        mock_logger_error.assert_called_once_with(
            "Configuration file mocked_path/uxr_config.json not found."
        )
    ...
    def test_save_config_success(
        self, mock_logger_info, mock_get_config_file, mock_open_file
    ):
        output_dir = "mocked_path"
        ...
        mock_logger_info.assert_called_once_with(
            "Configuration data saved to mocked_path/uxr_config.json"
        )
    ...
    @patch("uxr_analysis.config_manager.logger.error")
    def test_save_config_exception(
        self, mock_logger_error, mock_get_config_file, mock_open_file
    ):
        output_dir = "mocked_path"
        data = {"key": "value"}
    ...
    @patch(
        "uxr_analysis.config_manager.ConfigManager.get_latest_output_dir",
        return_value="mocked_dir",
    )
    @patch("uxr_analysis.config_manager.os.path.exists", return_value=True)
    @patch("builtins.input", return_value="yes")
    def test_confirm_latest_output_dir_yes(
        self, mock_input, mock_exists, mock_get_latest
    ):
        ...
        mock_input.assert_called_once_with(
            "Do you want to use the latest output directory (mocked_dir)? (yes/no): "
        )
    ...
    def test_confirm_latest_output_dir_no(
        self, mock_input, mock_exists, mock_get_latest
    ):
        result = ConfigManager.confirm_latest_output_dir()
    ...
    def test_update_latest_output_dir(self, mock_logger_info, mock_open_file):
        ...
        mock_logger_info.assert_called_once_with(
            f"Updated latest output directory to {output_dir}"
        )
